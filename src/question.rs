        ...
        match question.question_type {
            QuestionType::Int => {
                match input.parse::<i32>() {
                    // We want to specify i32
                    Ok(num) => return Ok(AnswerType::Int(num)),
                    Err(_e) => continue,
                }
            }
            QuestionType::Float => match input.parse::<f64>() {
                Ok(num) => return Ok(AnswerType::Float(num)),
                Err(_e) => continue,
            },
            QuestionType::PosNumber => match input.parse::<u32>() {
            ...
            QuestionType::Bool => match input.to_lowercase().as_str() {
...
pub fn create_new_question() -> Result<Question, std::io::Error> {
    loop {
        print!("What would you like your next Question to be: ");
        ...
        loop {
            print!("\nWhat would type do you want the answer to be: ");
            io::stdout().flush().unwrap();
